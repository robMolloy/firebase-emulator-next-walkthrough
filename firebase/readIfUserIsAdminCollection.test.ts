...
describe("firestore rules for a readAndCreateCollection", () => {
  ...
  it("should allow a get from a readAndCreateCollection", async () => {
    ...
    const authedDb2 = testEnv.authenticatedContext(uid2).firestore();
    const unauthedDb = testEnv.unauthenticatedContext().firestore();

    await testEnv.withSecurityRulesDisabled(async (context) => {
      const db = context.firestore();
      const docRef = doc(db, "readIfUserIsAdmin", "docId1");
      await setDoc(docRef, { some: "data", uid });

      const userRef = doc(db, "users", uid);
      await setDoc(userRef, { uid, isAdmin: true });

      const docRef2 = doc(db, "readIfUserIsAdmin", "docId2");
      await setDoc(docRef2, { some: "data", uid2 });
      const userRef2 = doc(db, "users", uid2);
      await setDoc(userRef2, { uid, isAdmin: false });
    });

    const unauthedDocRef = doc(unauthedDb, "readIfUserIsAdmin", "docId1");
    await assertFails(getDoc(unauthedDocRef));

    const authedDocRef = doc(authedDb, "readIfUserIsAdmin", "docId1");
    await assertSucceeds(getDoc(authedDocRef));
    const authedDocRef2 = doc(authedDb, "readIfUserIsAdmin", "docId2");
    await assertFails(getDoc(authedDocRef2));
  });
